...
#        耦合程序输入卡
##########################################################################################################
relaxation_factor = 1       #picard迭代松弛因子 1:直接代值，0.5：中值
...
coolant_T0 = 600.0  #初始化冷却剂温度值
coolant_D0 = 0.745  #初始化冷却剂密度，单位g/cm3
liner_power_start_insert_point = "$ card 5" #填入记得空格  $+空格+card+number
liner_power_end_insert_point = "$  card 7" #填入记得两个空格
core_power = 17670000/4 #总功率
n_depl = 3600 #燃耗区数目
n_pin_in_assembly = 72
n_eq_pin_in_assembly = 72 #组件模式采用集中参数为1，棒模式为组件内实际棒的个数
n_edge_channels_in_assembly = 8 #铅冷快堆组件内边通道数，压水堆对应GT外的冷却剂通道
n_corner_channels_in_assembly = 1 #铅冷快堆组件内角通道数，压水堆对应IT外的冷却剂通道
n_fuel_assembly = 1 #单层组件数目
#由于cobra必须输入完整的棒功率，但涉及如四分之一对称模型有半棒，或半组件，openmc计算的四分之一模型功率仅输出一半的，所以对应棒位置要乘2
half_pin_indices = [1,10,26,34,49,58,67,68,69,70,71,72]
half_pin_indices = [x - 1 for x in half_pin_indices]
half_assembly_indices = []
half_assembly_indices = [x - 1 for x in half_assembly_indices]
n_fuel_diff_type= [1] #单层燃料组件123的个数
n_nofuel_assembly = 0
n_floor = 50
n_type_fuel = 1
core_map_1 = [[1]*1]
...
fuel_1_start_tally_number = 2 #第一层的第一种燃料棒级tally开始的号，对应tally8为第一层第二种，以此类推，层层向上
#z_pin_1 = [[1]*50]
#z_pin = [int(x) for item in z_pin_1 for x in item]
...
#h_pin_1 = [[1]*50]
#h_pin = [int(x) for item in h_pin_1 for x in item]   #目前使用相同高度节块
...
###########################################################
run_openmc ="openmc > /dev/null 2>&1"#
run_cobra = "./EN"
coupled_mode = "pin"
...
statepoint_file = "statepoint.500.h5" #点状态文件名
fuel_1_name = 'fuel_1'
fuel_2_name = 'fuel_2'
fuel_3_name = 'fuel_3'
fuel_4_name = 'fuel_4'
...
if n_type_fuel == 1 :
	...
	if coupled_mode == "pin":
		fuel_1_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_2_assembly_coolant_e_T0 = np.array([])
		fuel_3_assembly_coolant_e_T0 = np.array([])
		fuel_4_assembly_coolant_e_T0 = np.array([])
		fuel_5_assembly_coolant_e_T0 = np.array([])
		fuel_1_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_2_assembly_coolant_c_T0 = np.array([])
		fuel_3_assembly_coolant_c_T0 = np.array([])
		fuel_4_assembly_coolant_c_T0 = np.array([])
		fuel_5_assembly_coolant_c_T0 = np.array([])
elif n_type_fuel == 2 :
	fuel_1_assembly_coolant_T0 = np.array([coolant_T0] * n_fuel_diff_type[0] * n_floor*n_eq_pin_in_assembly)
	...
	if coupled_mode == "pin":
		fuel_1_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_2_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_3_assembly_coolant_e_T0 = np.array([])
		fuel_4_assembly_coolant_e_T0 = np.array([])
		fuel_5_assembly_coolant_e_T0 = np.array([])
		fuel_1_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_2_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_3_assembly_coolant_c_T0 = np.array([])
		fuel_4_assembly_coolant_c_T0 = np.array([])
		fuel_5_assembly_coolant_c_T0 = np.array([])
elif n_type_fuel == 3 :
	...
	if coupled_mode == "pin":
		fuel_1_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_2_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_3_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_4_assembly_coolant_e_T0 = np.array([])
		fuel_5_assembly_coolant_e_T0 = np.array([])
		fuel_1_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_2_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_3_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_4_assembly_coolant_c_T0 = np.array([])
		fuel_5_assembly_coolant_c_T0 = np.array([])
elif n_type_fuel == 4 :
	fuel_1_assembly_coolant_T0 = np.array([coolant_T0] * n_fuel_diff_type[0] * n_floor*n_eq_pin_in_assembly)
	...
	if coupled_mode == "pin":
		...
		fuel_2_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_3_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_4_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_5_assembly_coolant_e_T0 = np.array([])
		fuel_1_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_2_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_3_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_4_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_5_assembly_coolant_c_T0 = np.array([])
elif n_type_fuel == 5 :
	fuel_1_assembly_coolant_T0 = np.array([coolant_T0] * n_fuel_diff_type[0] * n_floor*n_eq_pin_in_assembly)
	fuel_2_assembly_coolant_T0 = np.array([coolant_T0] * n_fuel_diff_type[1] * n_floor*n_eq_pin_in_assembly)
	...
	if coupled_mode == "pin":
		...
		fuel_3_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_4_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_5_assembly_coolant_e_T0 = np.array([coolant_T0] * n_floor * n_edge_channels_in_assembly)
		fuel_1_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_2_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_3_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_4_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
		fuel_5_assembly_coolant_c_T0 = np.array([coolant_T0] * n_floor * n_corner_channels_in_assembly)
else :
	print('初始化冷却剂温度错误')
##############################################################################################################
#        冷却剂密度初始化
...
pin_powers0 = np.full((int(n_fuel_assembly*n_pin_in_assembly), n_floor), core_power/n_floor/n_fuel_assembly/n_pin_in_assembly)
##################################################################################################################
#      openmc初步运行试算
##################################################################################################################
...
with h5py.File(statepoint_file, 'r') as hdf5_file:
	floor = 1
	data = np.zeros((max(n_fuel_diff_type) * n_pin_in_assembly, n_floor * n_type_fuel))
	while floor <= n_floor :
		fuel_type = 1
		while fuel_type <= n_type_fuel :
			data_0 = np.array(hdf5_file['tallies/tally '+str(fuel_1_start_tally_number + (floor-1)*n_type_fuel + fuel_type - 1)+'/results'][:,0])[:,0]
			data[0:n_fuel_diff_type[fuel_type-1]*n_pin_in_assembly,n_type_fuel*(floor-1)+fuel_type-1] = data_0
			fuel_type += 1
		floor += 1
	#data = np.array(data)
	#rows = data.shape
...
##########################################################
#二维功率系数数组转一维列数组并进行规范化顺序调整
##########################################################
	floor = 1
	data_c = []   #rows行，columns为列
	while floor <= n_floor :
		i = 0
		j = 0
		k = 0
		...
		while i <= len(core_map)-1 :
			if core_map[i] == 0:
				...
				fuel_type = 1
				subset = data[j:j+n_pin_in_assembly,n_type_fuel*(floor-1)+fuel_type-1]
				data_c.extend(subset)
				j += n_pin_in_assembly
			elif core_map[i] == 2:
				fuel_type = 2
				subset = data[k:k+n_pin_in_assembly,n_type_fuel*(floor-1)+fuel_type-1]
				data_c.extend(subset)
				k += n_pin_in_assembly
			elif core_map[i] == 3:
				fuel_type = 3
				subset = data[l:l+n_pin_in_assembly,n_type_fuel*(floor-1)+fuel_type-1]
				data_c.extend(subset)
				l += n_pin_in_assembly
			elif core_map[i] == 4:
				fuel_type = 4
				subset = data[m:m+n_pin_in_assembly,n_type_fuel*(floor-1)+fuel_type-1]
				data_c.extend(subset)
				m += n_pin_in_assembly
			elif core_map[i] == 5:
				fuel_type = 5
				subset = data[n:n+n_pin_in_assembly,n_type_fuel*(floor-1)+fuel_type-1]
				data_c.extend(subset)
				n += n_pin_in_assembly
			else:
				print(f"core_map错误")
	...
	data_c /= np.mean(data_c[data_c > 0.]) #归一化，note：燃耗区必须要相等
	data_c = core_power/n_depl*data_c #节块功率
	#data_c = np.array([f"{x:.5e}" for x in data_c])	#格式化数据
	#rows = data_c.shape
	#print(data_c)
	#print(f"数组有 {rows} 行，列")
##################################################################################################
#棒或者组件线功率分层写入cobra输入卡
##################################################################################################
floor = 1
i = 0
pin_powers = np.zeros((int(len(data_c)/n_floor), n_floor))
pin_liner_powers = np.zeros((int(len(data_c)/n_floor), n_floor))
assembly_powers = np.zeros((int(len(data_c)/n_floor/n_pin_in_assembly), n_floor))
assembly_liner_powers = np.zeros((int(len(data_c)/n_floor/n_pin_in_assembly), n_floor))
while floor <= n_floor :
	pin_powers[:,floor-1] = data_c[int(i):int(i+len(data_c)/n_floor)]
	pin_liner_powers[:,floor-1] = pin_powers[:,floor-1]/h_pin[floor-1]
	pin_liner_powers[:,floor-1][half_pin_indices] *= 2
	j = 0
	k = 0
	while k <= n_fuel_assembly - 1:
		assembly_powers[k, floor - 1] = pin_powers[j:j+n_pin_in_assembly,floor-1].sum(axis=0)
		assembly_liner_powers[k, floor - 1] = assembly_powers[k, floor - 1]/h_pin[floor-1]
		j += n_pin_in_assembly
		k += 1
...
pin_liner_powers = np.array([[f"{x:.5e}" for x in row] for row in pin_liner_powers])  #注意：此为二维数据格式化,科学计数法且保留五位小数
...
if coupled_mode == "pin":
	...
	if start_index is None or end_index is None:  # 检查是否找到两行
		...
		while floor <= n_floor:
			...
			for i, item in enumerate(pin_liner_powers[:, floor - 1]):
				new_content += str(item) + " "
				if (i + 1) % 6 == 0:  # 每六个元素一换行
					new_content += "\n"
			if n_pin_in_assembly * n_fuel_assembly % 6 != 0:
				new_content += "\n"
			floor += 1
...
# for i, rod_temps in enumerate(temperature_2d_array):
...
# 	print("Distance (m):", channels[channel]['distance'])
# 	print("Temperature (°C):", channels[channel]['temperature'])
# 	print("Density (kg/m³):", channels[channel]['density'])
# 	print()
...
# # 打印二维数组
# print("Temperature 2D array:")
# for row in coolant_temperature_2d_array:
# 	print(row)
# print("\nDensity 2D array:")
# for row in coolant_density_2d_array:
# 	print(row)
#####################################################################################################
...
fuel_1_assembly_coolant_e_T = []
fuel_1_assembly_coolant_c_T = []
fuel_2_assembly_coolant_e_T = []
fuel_2_assembly_coolant_c_T = []
fuel_3_assembly_coolant_e_T = []
fuel_3_assembly_coolant_c_T = []
fuel_4_assembly_coolant_e_T = []
fuel_4_assembly_coolant_c_T = []
fuel_5_assembly_coolant_e_T = []
fuel_5_assembly_coolant_c_T = []
while floor <= n_floor :
	i = 0
	j = 0
	while i <= len(core_map)-1 :
		if core_map[i] == 0:
			...
			while k < n_eq_pin_in_assembly:
				...
				j += 1
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_1_assembly_coolant_e_T.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_1_assembly_coolant_c_T.append(subset)
					j += 1
					k += 1
		elif core_map[i] == 2:
			k = 0
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_2_assembly_coolant_e_T.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_2_assembly_coolant_c_T.append(subset)
					j += 1
					k += 1
		elif core_map[i] == 3:
			k = 0
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_3_assembly_coolant_e_T.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_3_assembly_coolant_c_T.append(subset)
					j += 1
					k += 1
		elif core_map[i] == 4:
			k = 0
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_4_assembly_coolant_e_T.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_4_assembly_coolant_c_T.append(subset)
					j += 1
					k += 1
		elif core_map[i] == 5:
			k = 0
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_5_assembly_coolant_e_T.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_temperature_2d_array[j, floor - 1]
					fuel_5_assembly_coolant_c_T.append(subset)
					j += 1
					k += 1
		else:
			print(f"读取燃料温度建立的数组按照堆芯燃料类型和排序（coremap）规范化处理错误")
...
fuel_1_assembly_coolant_e_T = np.array(fuel_1_assembly_coolant_e_T)
fuel_1_assembly_coolant_c_T = np.array(fuel_1_assembly_coolant_c_T)
fuel_2_assembly_coolant_e_T = np.array(fuel_2_assembly_coolant_e_T)
fuel_2_assembly_coolant_c_T = np.array(fuel_2_assembly_coolant_c_T)
fuel_3_assembly_coolant_e_T = np.array(fuel_3_assembly_coolant_e_T)
fuel_3_assembly_coolant_c_T = np.array(fuel_3_assembly_coolant_c_T)
fuel_4_assembly_coolant_e_T = np.array(fuel_4_assembly_coolant_e_T)
fuel_4_assembly_coolant_c_T = np.array(fuel_4_assembly_coolant_c_T)
fuel_5_assembly_coolant_e_T = np.array(fuel_5_assembly_coolant_e_T)
fuel_5_assembly_coolant_c_T = np.array(fuel_5_assembly_coolant_c_T)
#####################################################################################################
...
fuel_1_assembly_coolant_e_D = []
fuel_1_assembly_coolant_c_D = []
fuel_2_assembly_coolant_e_D = []
fuel_2_assembly_coolant_c_D = []
fuel_3_assembly_coolant_e_D = []
fuel_3_assembly_coolant_c_D = []
fuel_4_assembly_coolant_e_D = []
fuel_4_assembly_coolant_c_D = []
fuel_5_assembly_coolant_e_D = []
fuel_5_assembly_coolant_c_D = []
while floor <= n_floor :
	i = 0
	j = 0
	while i <= len(core_map)-1 :
		if core_map[i] == 0:
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_1_assembly_coolant_e_D.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_1_assembly_coolant_c_D.append(subset)
					j += 1
					k += 1
					#print(fuel_1_assembly_coolant_c_D)
		elif core_map[i] == 2:
			k = 0
			while k < n_eq_pin_in_assembly:
				subset = coolant_density_2d_array[j,floor-1]
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_2_assembly_coolant_e_D.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_2_assembly_coolant_c_D.append(subset)
					j += 1
					k += 1
		elif core_map[i] == 3:
			k = 0
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_3_assembly_coolant_e_D.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_3_assembly_coolant_c_D.append(subset)
					j += 1
					k += 1
		elif core_map[i] == 4:
			k = 0
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_4_assembly_coolant_e_D.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_4_assembly_coolant_c_D.append(subset)
					j += 1
					k += 1
		elif core_map[i] == 5:
			k = 0
			...
			if coupled_mode == "pin":
				while k >= n_eq_pin_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_5_assembly_coolant_e_D.append(subset)
					j += 1
					k += 1
				while k >= n_eq_pin_in_assembly + n_edge_channels_in_assembly and k < n_eq_pin_in_assembly + n_edge_channels_in_assembly + n_corner_channels_in_assembly:
					subset = coolant_density_2d_array[j, floor - 1]
					fuel_5_assembly_coolant_c_D.append(subset)
					j += 1
					k += 1
		else:
			print(f"读取燃料温度建立的数组按照堆芯燃料类型和排序（coremap）规范化处理错误")
...
fuel_1_assembly_coolant_e_D = np.array(fuel_1_assembly_coolant_e_D)
fuel_1_assembly_coolant_c_D = np.array(fuel_1_assembly_coolant_c_D)
fuel_2_assembly_coolant_e_D = np.array(fuel_2_assembly_coolant_e_D)
fuel_2_assembly_coolant_c_D = np.array(fuel_2_assembly_coolant_c_D)
fuel_3_assembly_coolant_e_D = np.array(fuel_3_assembly_coolant_e_D)
fuel_3_assembly_coolant_c_D = np.array(fuel_3_assembly_coolant_c_D)
fuel_4_assembly_coolant_e_D = np.array(fuel_4_assembly_coolant_e_D)
fuel_4_assembly_coolant_c_D = np.array(fuel_4_assembly_coolant_c_D)
fuel_5_assembly_coolant_e_D = np.array(fuel_5_assembly_coolant_e_D)
fuel_5_assembly_coolant_c_D = np.array(fuel_5_assembly_coolant_c_D)
...
#print(fuel_1_assembly_T)
fuel_1_assembly_coolant_e_D1 = fuel_1_assembly_coolant_e_D
fuel_1_assembly_coolant_c_D1 = fuel_1_assembly_coolant_c_D
fuel_2_assembly_coolant_e_D1 = fuel_2_assembly_coolant_e_D
fuel_2_assembly_coolant_c_D1 = fuel_2_assembly_coolant_c_D
fuel_3_assembly_coolant_e_D1 = fuel_3_assembly_coolant_e_D
fuel_3_assembly_coolant_c_D1 = fuel_3_assembly_coolant_c_D
fuel_4_assembly_coolant_e_D1 = fuel_4_assembly_coolant_e_D
fuel_4_assembly_coolant_c_D1 = fuel_4_assembly_coolant_c_D
fuel_5_assembly_coolant_e_D1 = fuel_5_assembly_coolant_e_D
fuel_5_assembly_coolant_c_D1 = fuel_5_assembly_coolant_c_D

fuel_1_assembly_coolant_e_T1 = fuel_1_assembly_coolant_e_T
fuel_1_assembly_coolant_c_T1 = fuel_1_assembly_coolant_c_T
fuel_2_assembly_coolant_e_T1 = fuel_2_assembly_coolant_e_T
fuel_2_assembly_coolant_c_T1 = fuel_2_assembly_coolant_c_T
fuel_3_assembly_coolant_e_T1 = fuel_3_assembly_coolant_e_T
fuel_3_assembly_coolant_c_T1 = fuel_3_assembly_coolant_c_T
fuel_4_assembly_coolant_e_T1 = fuel_4_assembly_coolant_e_T
fuel_4_assembly_coolant_c_T1 = fuel_4_assembly_coolant_c_T
fuel_5_assembly_coolant_e_T1 = fuel_5_assembly_coolant_e_T
fuel_5_assembly_coolant_c_T1 = fuel_5_assembly_coolant_c_T
#####################################################################################################
#      燃料和冷却剂的picard迭代
#####################################################################################################
...
if coupled_mode == "pin":
	fuel_1_assembly_coolant_e_D = fuel_1_assembly_coolant_e_D0 + relaxation_factor*(fuel_1_assembly_coolant_e_D1 - fuel_1_assembly_coolant_e_D0)
	fuel_1_assembly_coolant_c_D = fuel_1_assembly_coolant_c_D0 + relaxation_factor*(fuel_1_assembly_coolant_c_D1 - fuel_1_assembly_coolant_c_D0)
	fuel_2_assembly_coolant_e_D = fuel_2_assembly_coolant_e_D0 + relaxation_factor*(fuel_2_assembly_coolant_e_D1 - fuel_2_assembly_coolant_e_D0)
	fuel_2_assembly_coolant_c_D = fuel_2_assembly_coolant_c_D0 + relaxation_factor*(fuel_2_assembly_coolant_c_D1 - fuel_2_assembly_coolant_c_D0)
	fuel_3_assembly_coolant_e_D = fuel_3_assembly_coolant_e_D0 + relaxation_factor*(fuel_3_assembly_coolant_e_D1 - fuel_3_assembly_coolant_e_D0)
	fuel_3_assembly_coolant_c_D = fuel_3_assembly_coolant_c_D0 + relaxation_factor*(fuel_3_assembly_coolant_c_D1 - fuel_3_assembly_coolant_c_D0)
	fuel_4_assembly_coolant_e_D = fuel_4_assembly_coolant_e_D0 + relaxation_factor*(fuel_4_assembly_coolant_e_D1 - fuel_4_assembly_coolant_e_D0)
	fuel_4_assembly_coolant_c_D = fuel_4_assembly_coolant_c_D0 + relaxation_factor*(fuel_4_assembly_coolant_c_D1 - fuel_4_assembly_coolant_c_D0)
	fuel_5_assembly_coolant_e_D = fuel_5_assembly_coolant_e_D0 + relaxation_factor*(fuel_5_assembly_coolant_e_D1 - fuel_5_assembly_coolant_e_D0)
	fuel_5_assembly_coolant_c_D = fuel_5_assembly_coolant_c_D0 + relaxation_factor*(fuel_5_assembly_coolant_c_D1 - fuel_5_assembly_coolant_c_D0)
...
if n_type_fuel == 1 :
	...
	if coupled_mode == "pin":
		coolant_all_assembly_T1 = np.concatenate((fuel_1_assembly_coolant_T1,fuel_1_assembly_coolant_e_T1,fuel_1_assembly_coolant_c_T1))
	coolant_all_assembly_T0 = np.concatenate((fuel_1_assembly_coolant_T0,))
	if coupled_mode == "pin":
		coolant_all_assembly_T0 = np.concatenate((fuel_1_assembly_coolant_T0,fuel_1_assembly_coolant_e_T0,fuel_1_assembly_coolant_c_T0))
...
if coupled_mode == "pin":
	abs_error = np.abs(pin_powers - pin_powers0)
	eps = 1e-12
	relative_error = abs_error / (np.abs(pin_powers0) + eps)  # 计算相对误差（避免除以零）
elif coupled_mode == "assembly":
...
while  fuel_temp_condition_max or coolant_temp_condition_max or power_condition_max :
	...
	fuel_1_assembly_coolant_e_D0 = fuel_1_assembly_coolant_e_D1
	fuel_1_assembly_coolant_c_D0 = fuel_1_assembly_coolant_c_D1
	fuel_2_assembly_coolant_e_D0 = fuel_2_assembly_coolant_e_D1
	fuel_2_assembly_coolant_c_D0 = fuel_2_assembly_coolant_c_D1
	fuel_3_assembly_coolant_e_D0 = fuel_3_assembly_coolant_e_D1
	fuel_3_assembly_coolant_c_D0 = fuel_3_assembly_coolant_c_D1
	fuel_4_assembly_coolant_e_D0 = fuel_4_assembly_coolant_e_D1
	fuel_4_assembly_coolant_c_D0 = fuel_4_assembly_coolant_c_D1
	fuel_5_assembly_coolant_e_D0 = fuel_5_assembly_coolant_e_D1
	fuel_5_assembly_coolant_c_D0 = fuel_5_assembly_coolant_c_D1

	fuel_1_assembly_coolant_e_T0 = fuel_1_assembly_coolant_e_T1
	fuel_1_assembly_coolant_c_T0 = fuel_1_assembly_coolant_c_T1
	fuel_2_assembly_coolant_e_T0 = fuel_2_assembly_coolant_e_T1
	fuel_2_assembly_coolant_c_T0 = fuel_2_assembly_coolant_c_T1
	fuel_3_assembly_coolant_e_T0 = fuel_3_assembly_coolant_e_T1
	fuel_3_assembly_coolant_c_T0 = fuel_3_assembly_coolant_c_T1
	fuel_4_assembly_coolant_e_T0 = fuel_4_assembly_coolant_e_T1
	fuel_4_assembly_coolant_c_T0 = fuel_4_assembly_coolant_c_T1
	fuel_5_assembly_coolant_e_T0 = fuel_5_assembly_coolant_e_T1
	fuel_5_assembly_coolant_c_T0 = fuel_5_assembly_coolant_c_T1
	...
	if coupled_mode == "pin":
		...
		while j < n_edge_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_1_assembly_coolant_e_materials):  # 确保索引不越界
				fuel_1_assembly_coolant_e_materials[i].set('temperature', str(fuel_1_assembly_coolant_e_T[k]))
				density = fuel_1_assembly_coolant_e_materials[i].find('density')
				density.set('value', str(fuel_1_assembly_coolant_e_D[k]))
			i += 1
			j += 1
			k += 1
		# 只处理fuel_2材料
		...
		while j < n_edge_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_2_assembly_coolant_e_materials):  # 确保索引不越界
				fuel_2_assembly_coolant_e_materials[i].set('temperature', str(fuel_2_assembly_coolant_e_T[k]))
				density = fuel_2_assembly_coolant_e_materials[i].find('density')
				density.set('value', str(fuel_2_assembly_coolant_e_D[k]))
			i += 1
			j += 1
			k += 1
		# 只处理fuel_3材料
		...
		while j < n_edge_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_3_assembly_coolant_e_materials):  # 确保索引不越界
				fuel_3_assembly_coolant_e_materials[i].set('temperature', str(fuel_3_assembly_coolant_e_T[k]))
				density = fuel_3_assembly_coolant_e_materials[i].find('density')
				density.set('value', str(fuel_3_assembly_coolant_e_D[k]))
			i += 1
			j += 1
			k += 1
		# 只处理fuel_4材料
		...
		while j < n_edge_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_4_assembly_coolant_e_materials):  # 确保索引不越界
				fuel_4_assembly_coolant_e_materials[i].set('temperature', str(fuel_4_assembly_coolant_e_T[k]))
				density = fuel_4_assembly_coolant_e_materials[i].find('density')
				density.set('value', str(fuel_4_assembly_coolant_e_D[k]))
			i += 1
			j += 1
			k += 1
		# 只处理fuel_5材料
		...
		while j < n_edge_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_5_assembly_coolant_e_materials):  # 确保索引不越界
				fuel_5_assembly_coolant_e_materials[i].set('temperature', str(fuel_5_assembly_coolant_e_T[k]))
				density = fuel_5_assembly_coolant_e_materials[i].find('density')
				density.set('value', str(fuel_5_assembly_coolant_e_D[k]))
			i += 1
			j += 1
			k += 1
		#################################################################################################################
		...
		while j < n_corner_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_1_assembly_coolant_c_materials):  # 确保索引不越界
				fuel_1_assembly_coolant_c_materials[i].set('temperature', str(fuel_1_assembly_coolant_c_T[k]))
				density = fuel_1_assembly_coolant_c_materials[i].find('density')
				density.set('value', str(fuel_1_assembly_coolant_c_D[k]))
			i += 1
			j += 1
			k += 1
		# 只处理fuel_2材料
		...
		while j < n_corner_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_2_assembly_coolant_c_materials):  # 确保索引不越界
				fuel_2_assembly_coolant_c_materials[i].set('temperature', str(fuel_2_assembly_coolant_c_T[k]))
				density = fuel_2_assembly_coolant_c_materials[i].find('density')
				density.set('value', str(fuel_2_assembly_coolant_c_D[k]))
			i += 1
			j += 1
			k += 1
		# 只处理fuel_3材料
		...
		while j < n_corner_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_3_assembly_coolant_c_materials):  # 确保索引不越界
				fuel_3_assembly_coolant_c_materials[i].set('temperature', str(fuel_3_assembly_coolant_c_T[k]))
				density = fuel_3_assembly_coolant_c_materials[i].find('density')
				density.set('value', str(fuel_3_assembly_coolant_c_D[k]))
			i += 1
			j += 1
			k += 1
		...
		while j < n_corner_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_4_assembly_coolant_c_materials):  # 确保索引不越界
				fuel_4_assembly_coolant_c_materials[i].set('temperature', str(fuel_4_assembly_coolant_c_T[k]))
				density = fuel_4_assembly_coolant_c_materials[i].find('density')
				density.set('value', str(fuel_4_assembly_coolant_c_D[k]))
			i += 1
			j += 1
			k += 1
		...
		while j < n_corner_channels_in_assembly * n_fuel_assembly * n_floor:
			if i < len(fuel_5_assembly_coolant_c_materials):  # 确保索引不越界
				fuel_5_assembly_coolant_c_materials[i].set('temperature', str(fuel_5_assembly_coolant_c_T[k]))
				density = fuel_5_assembly_coolant_c_materials[i].find('density')
				density.set('value', str(fuel_5_assembly_coolant_c_D[k]))
			i += 1
			j += 1
			k += 1
	...
	if n_type_fuel == 1:
		...
		if coupled_mode == "pin":
			coolant_all_assembly_T1 = np.concatenate(
				(fuel_1_assembly_coolant_T1, fuel_1_assembly_coolant_e_T1, fuel_1_assembly_coolant_c_T1))
		coolant_all_assembly_T0 = np.concatenate((fuel_1_assembly_coolant_T0,))
		if coupled_mode == "pin":
			coolant_all_assembly_T0 = np.concatenate(
				(fuel_1_assembly_coolant_T0, fuel_1_assembly_coolant_e_T0, fuel_1_assembly_coolant_c_T0))
		coolant_temp_rmse = np.sqrt(np.mean(np.abs(coolant_all_assembly_T1 - coolant_all_assembly_T0) ** 2))
		coolant_temp_condition = coolant_temp_rmse > coolant_temp_convergence_limit
		max_fuel_convergence = np.linalg.norm(fuel_1_assembly_T1 - fuel_1_assembly_T0, ord=np.inf)
		max_coolant_convergence = np.linalg.norm(fuel_1_assembly_coolant_T1 - fuel_1_assembly_coolant_T0, ord=np.inf)
...
if coupled_mode == "pin":
	print("\n  内容太多，不便输出，请到输出文件查找!")
	# print("高度位置:", height_positions)
	# print("燃料棒编号:", rod_numbers)
	# print("\n二维数组（行=燃料棒，列=高度，单位：K）:")
	# for i, rod_temps in enumerate(all_fuel_T_xH_yN):
	# 	print(f"燃料棒 {rod_numbers[i]}: {rod_temps}")
elif coupled_mode == "assembly":
	print("高度位置:", height_positions)
	print("燃料组件编号:", rod_numbers)
	print("\n二维数组（行=燃料组件，列=高度，单位：K）:")
